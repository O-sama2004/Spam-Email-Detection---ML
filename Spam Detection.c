...
void ADAM(int **hotVectors, int *labels, int wordsSize, int lines, int fileIndex, double initialValue);
...
void GD(int **hotVectors, int *labels, int wordsSize, int lines,int fileIndex,double initialValue);
void SGD(int **hotVectors, int *labels, int wordsSize, int lines,int fileIndex,double initialValue);
...
int main() {
    ...
    readData(&hotVectors, &labels, &wordsSize, &lines);
    shuffleData(&hotVectors, &labels, lines);

    double initialValues[5]={0.001,0,0.1,0.5,1};
    for(int i=0;i<5;i++){
        
        GD(hotVectors, labels, wordsSize, lines,i+1,initialValues[i]);
        SGD(hotVectors, labels, wordsSize, lines,i+1,initialValues[i]);    
        ADAM(hotVectors, labels, wordsSize, lines,i+1,initialValues[i]);
    }
...
void SGD(int **hotVectors, int *labels, int wordsSize, int lines,int fileIndex,double initialValue) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {//for each epoch
        ...
        gettimeofday(&end, NULL);//calculate time for each epoch
        double time_taken = ((end.tv_sec - start.tv_sec) * 1000.0f + (end.tv_usec - start.tv_usec) / 1000.0f) / 1000.0f;
        ...
        double accuracy = (double)correct_predictions / (batch_size);
        printf("Epoch %d: Loss = %f, Accuracy = %f\n", epoch, losses[epoch], accuracy);
    }

    char filename[100];
    sprintf(filename, "C:\\Users\\BTK\\Desktop\\diferansiyel\\SGD\\loss_time_epocks\\gd_performance%d.csv", fileIndex);//write losses to csv files
    ...
    for(int i=train_size+1;i<lines;i++){
        ...
        // for (int j = 0; j < wordsSize; j++) {
        //     weights[j] -= learning_rate * error * tanh_derivative(prediction) * hotVectors[i][j];//wn+1=wn-learning_rate*2*(y-tanh(w*xi))*tanh'(w*xi)*xi
        // }

        total_loss += mean_squared_error(&labels[i], &prediction, 1);//total loss function for each epoch

        if ((prediction >= 0.0 && labels[i] == 1) || (prediction < 0.0 && labels[i] == -1)) {
    ...
    double avg_loss = total_loss / (lines-train_size);
    double accuracy = (double)correct_predictions / (lines-train_size);
...
void ADAM(int **hotVectors, int *labels, int wordsSize, int lines, int fileIndex, double initialValue) {
    ...
    double beta2 = 0.999;
    double epsilon = 1e-8;
    double *m = calloc(wordsSize, sizeof(double));
    double *v = calloc(wordsSize, sizeof(double));
    ...
    double **epochWeights = malloc(epochs * sizeof(double *));
    for (int i = 0; i < epochs; i++) {
        epochWeights[i] = malloc(wordsSize * sizeof(double));
    }
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {//for each epoch
        ...
        for (int i = 0; i < batch_size; i++) {//for each training data
            ...
            for (int j = 0; j < wordsSize; j++) {
                ...
                v[j] = beta2 * v[j] + (1 - beta2) * gradient * gradient;//v=beta2*v+(1-beta2)*gradient^2

                // double m_hat = m[j] / (1 - pow(beta1, epoch + 1));//momentum vector m_hat=m/(1-beta1^(epoch+1))
                // double v_hat = v[j] / (1 - pow(beta2, epoch + 1));//velocity vector v_hat=v/(1-beta2^(epoch+1))


                double m_hat = m[j] ;//momentum vector m_hat=m/(1-beta1^(epoch+1))
                double v_hat = v[j] ;//velocity vector v_hat=v/(1-beta2^(epoch+1))

                
                weights[j] -= learning_rate * m_hat / (sqrt(v_hat + epsilon));
            }

            total_loss += mean_squared_error(&labels[index], &prediction, 1);
            if ((prediction >= 0 && labels[index] == 1) || (prediction < 0 && labels[index] == -1)) {
                correct_predictions++;
            }
        }
    ...
    // ... Same code for writing CSV and Testing as in GD function ...
...
void GD(int **hotVectors, int *labels, int wordsSize, int lines,int fileIndex,double initialValue) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {//for each epoch
        ...
        double accuracy = (double)correct_predictions / train_size;
        printf("Epoch %d: Loss = %f, Accuracy = %f\n", epoch, losses[epoch], accuracy);
    }
    ...
    while (fgets(buf, 10000, fp) != NULL) {
        ...
        while (token != NULL) {
            ...
            if (nextToken != NULL) {
                ...
                *labels = realloc(*labels, (labelsSize + 1) * sizeof(int));
                (*labels)[labelsSize++] = atoi(token);
                break;
            }
        }

        *hotVectors = realloc(*hotVectors, (*lines + 1) * sizeof(int *));
        (*hotVectors)[(*lines)++] = hotVector;
    }
